...
%! endfunction
%!
%! function modal = create_modal_data_mbdyn(mesh, dof_map, load_case_dof, load_case_stat, options, param)
%!   filename = "";
%!   filename = tempname();
%!   if (ispc())
%!     filename(filename == "\\") = "/";
...
%!       shell(sprintf("cat \"%s\"", [options_mbd.output_file, ".out"]));
...
%! endfunction
%!
%! function [Freact, mesh, mesh_def, sol_stat, sol_eig, sol_eig_def, modal] = transmissibility(geometry, material, param)
...
%!   options_mbd.fmin = 10;
%!   options_mbd.fmax = max(sol_eig.f);
%!   options_mbd.verbose = true;
%!   modal = create_modal_data_mbdyn(mesh, dof_map, load_case_dof, load_case_stat, options_mbd, param);
...
%!   [Freact{i}, mesh, mesh_def, sol_stat, sol_eig, sol_eig_def, modal] = transmissibility(geometry(i), material, param);
%! endfor
%! figure("visible", "off");
%! hold on;
%! for i=1:numel(geometry)
