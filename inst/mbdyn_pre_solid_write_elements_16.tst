## mbdyn_pre_solid_write_elements.tst:16
%!test
%! try
%! ## TEST 9
...
%!   opt_msh.elem_type = {"iso4", "iso8f", "penta6f", "iso4"};
...
%!   if (isfield(mesh.elements, "iso8f"))
%!     mesh.materials.iso8f = zeros(rows(mesh.elements.iso8f), 1, "int32");
%!     mesh.materials.iso8f([mesh.groups.iso8f(find([[mesh.groups.iso8f.id] == 1])).elements]) = 1;
%!   endif
%!   if (isfield(mesh.elements, "penta6f"))
%!     mesh.materials.penta6f = zeros(rows(mesh.elements.penta6), 1, "int32");
%!     mesh.materials.penta6f([mesh.groups.penta6(find([[mesh.groups.penta6.id] == 1])).elements]) = 1;
%!   endif
