...
%! unwind_protect
%!   unwind_protect
%!     [fd, fname] = mkstemp(fullfile(tempdir(), "mbdyn_post_load_output_XXXXXX"));
%!     fputs(fd, " begin: data;\n");
...
%!     endif
%!   end_unwind_protect
%!   pid = spawn("mbdyn", {"--silent", "-f", fname, "-o", fname});
%!   status = spawn_wait(pid);
%!   if (status ~= 0)
%!     error("mbdyn failed with status %d", status);
%!   endif
%!   [t, dt, niter, reserr, solerr, solconv, output_flag] = mbdyn_post_load_output_out(fname, 1024, false);
%!   [node_id, node_data] = mbdyn_post_load_output([fname, ".mov"], 18, [1], numel(t), 18, 1, false);
%!   g = -9.81;
